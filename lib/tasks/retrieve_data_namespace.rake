...
task :retrieve_data, %i[page city_letter] => :environment do |_t, args|
  Rake::Task['fill_extras'].invoke if Extra.any? == false
  ...
  body.length.times do |item|
    ...
    @currency = if body[item]['prices_attributes'][0]['currency'] == 'EUR'
                  "\xE2\x82\xAC"
    ...
    @owner_gender = if body[item]['tenants'][0]['biological_sex'] == 1
                      'Female'
